...
// Server, Handler is non-nil, Close is a no-op when never used. Catching
// constructor regressions costs ~ms.
...
// Real ABS clients then post the "auth" event over the resulting sid.
...
// when no sockets are connected to the user's room. Realtime updates fire
// on every progress write — if a user has never opened the mobile app,
// every PATCH /me/progress would hit this path.
...
// TestEventPublisherInterfaceSatisfied is a compile-time check (executed
// at test time only because Go can't run free statements at file scope).
// It guarantees that abssocket.Server stays usable where abs.Handler
// expects an abs.EventPublisher, so the main.go wiring keeps compiling
// even if the EventPublisher interface gains methods later.
